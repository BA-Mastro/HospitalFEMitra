...
const CreateDoctor = () => {
    ...
    const { mutate } = useMutation({
        mutationFn: async (data: Doctor) => {
            const response = await axios.post("http://localhost:8080/api/doctor/create", data);
            return response.data;
        },
        onSuccess: () => {
            alert("Doctor has been successfully added.");
    ...
    const formik = useFormik({
        initialValues: {
            ...
            email: "",
            specialization: "",
            department_id: "" // Default empty selection
        },
        validationSchema: validationRules,
        onSubmit: (data) => {
            // Convert phone_number to a number while ensuring only digits are sent
            const formattedData = {
                ...data,
                phone_number: parseInt(data.phone_number.replace(/\D/g, ""), 10) // Remove non-digits
            };
